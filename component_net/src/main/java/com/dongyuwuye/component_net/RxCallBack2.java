 ...
 * email：
 */
public abstract class RxCallBack2<T> implements Observer<T> {
    ...
    public RxCallBack2(Context context) {
        mContext = context;
        dialog = new MProgressDialog(mContext, new MProgressDialog.Builder(mContext));
    }

    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    public RxCallBack2(Context context, String showTxt) {
        dialog = new MProgressDialog(mContext, new MProgressDialog.Builder(mContext));
...
//            }
//        }
        _onError(ExceptionHelper.handleException(e), e);
        onComplete();
    }
    ...
    public abstract void _onError(String e, Throwable throwable);
}
